...
// BUGGY VERSION - Will deadlock!
void transfer_buggy(account_t* from, account_t* to, int amount) {
...
// TODO: Implement a deadlock-free version
void transfer_safe(account_t* from, account_t* to, int amount) {
    // TODO: Lock accounts in a consistent order to prevent circular wait
    // Hint: Always lock the account with lower ID first
    // Example approach:
    ...
    // For now, just call the buggy version (YOU NEED TO FIX THIS)
    transfer_buggy(from, to, amount);
}
